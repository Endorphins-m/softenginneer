...
FONT_SIZE = 40
SMALL_FONT_SIZE = 20
custom_font = pygame.font.Font(FONT_NAME, FONT_SIZE)
...
# 初始化牌组函数
def init_tiles():
    global tiles, docks, time_left, game_over, game_started, difficulty, score
    tiles = []
    docks = []
    score = 0  # 每次开始新游戏时重置得分
    time_left = 120  # 重置时间
    # 根据难度选择生成不同层数的牌
    layers = 5
    pic_num = 5
...
def draw_start_screen():
    screen.clear()
    screen.blit('shadow', (0, 0))  # 开始界面的背景
    ...
    text_surface = custom_font.render("1. 简单模式", True, (255, 255, 255))
    screen.blit(text_surface, (WIDTH / 2 - text_surface.get_width() / 2, HEIGHT / 2 - 50))

    text_surface = custom_font.render("2. 中等模式", True, (255, 255, 255))
    screen.blit(text_surface, (WIDTH / 2 - text_surface.get_width() / 2, HEIGHT / 2))

    text_surface = custom_font.render("3. 困难模式", True, (255, 255, 255))
    screen.blit(text_surface, (WIDTH / 2 - text_surface.get_width() / 2, HEIGHT / 2 + 50))
...
def draw_game_screen():
    ...
    screen.draw.text(f"score: {score}", (10, 60), fontsize=40, color="white")
    ...
    if time_left == 0:
        ...
        screen.blit(text_surface, (WIDTH / 2 - text_surface.get_width() / 2, HEIGHT / 2 - 150))
        final_score = score  # 假设每秒时间对应2分
        screen.draw.text(f"Final Score: {final_score}", (10, 110), fontsize=40, color="red")
    # 游戏结束或胜利提示
    if len(docks) >= 7:
...
def on_mouse_down(pos):
    global game_started, docks, game_over, difficulty, in_menu,score
